...
391. **`version --output-format json` includes a `message` field containing human-readable prose (`"Claw Code\n  Version  0.1.0\n  Git SHA  ..."`) in addition to the structured fields `version`, `git_sha`, `target`; no `action` field; no `build_date`, `platform`, or `rustc_version` structured fields — the prose is embedded inside the JSON envelope instead of being derivable from the structured fields** — dogfooded 2026-04-30 by Jobdori on `e939777f`. Running `./claw --output-format json version` returns `{"git_sha":"5eb1d7d8","kind":"version","message":"Claw Code\n  Version  0.1.0\n  Git SHA  5eb1d7d8\n  Target  aarch64-apple-darwin\n  Build date  2026-04-30","target":"aarch64-apple-darwin","version":"0.1.0"}`. The `message` field duplicates structured data as prose, preventing automation from using the JSON without also parsing the prose for any field not promoted to a top-level key (e.g., `build_date`). No `action` field present (compare `agents/mcp` which include `action:"list"`). No `platform`, `rustc_version`, or `build_timestamp_ms` structured fields. **Required fix shape:** (a) remove or deprecate `message` prose field from `--output-format json` output; (b) add structured `build_date` (ISO-8601), `platform`, and `rustc_version` fields; (c) add `action:"version"` for envelope consistency; (d) add regression coverage proving the JSON response contains no prose-only fields. Source: Jobdori live dogfood, `e939777f`, 2026-04-30.
